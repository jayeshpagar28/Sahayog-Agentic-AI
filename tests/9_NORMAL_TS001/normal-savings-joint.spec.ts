...
/**
 * Normal Savings Account (1001) — Joint Account Type, complete live journey.
 *
 * Covers: user-stories/US_009_Normal_ Account_journey.md (all ACs for the Joint path) /
 * specs/NORMAL_TS001-test-plan.md. This is the full end-to-end flow — Mobile Verification
 * through real final Submit — driven against real UAT, not a mock. Normal Joint requires
 * Introducer Details.
 *
...
const OTP_SOURCE: OtpSource = {
  literal: process.env.SAHAYOG_NOR_JNT_OTP,
  url: process.env.SAHAYOG_NOR_JNT_OTP_URL,
  signalFile: path.join(process.cwd(), '.nor-jnt-otp-input.txt'),
};
const CO_OTP_SOURCE: OtpSource = {
  literal: process.env.SAHAYOG_NOR_JNT_CO_OTP,
  url: process.env.SAHAYOG_NOR_JNT_CO_OTP_URL,
  signalFile: path.join(process.cwd(), '.nor-jnt-co-otp-input.txt'),
};

test.describe('NORMAL_TS001 - Joint Account (Live)', () => {
  test.skip(
    !!process.env.CI && !(hasUnattendedOtp(OTP_SOURCE) && hasUnattendedOtp(CO_OTP_SOURCE)),
    'Needs an OTP source for both applicants to run unattended in CI. Set SAHAYOG_NOR_JNT_OTP/' +
      '_URL and SAHAYOG_NOR_JNT_CO_OTP/_URL; locally OTPs are read from .nor-jnt-otp-input.txt ' +
      'and .nor-jnt-co-otp-input.txt. (DigiLocker consent and Liveliness are resolved by polling ' +
      'application status, so they need no configuration.)',
  );

  test('Normal Savings Account - Joint: full live journey through real final submission', async ({ page }) => {
    const applicantMobile = process.env.SAHAYOG_NOR_JNT_MOBILE;
    const coApplicantMobile = process.env.SAHAYOG_NOR_JNT_CO_MOBILE;
    test.skip(
      !applicantMobile || !coApplicantMobile,
      'Set SAHAYOG_NOR_JNT_MOBILE and SAHAYOG_NOR_JNT_CO_MOBILE to two real, not-recently-used mobile numbers before running this flow.',
    );
