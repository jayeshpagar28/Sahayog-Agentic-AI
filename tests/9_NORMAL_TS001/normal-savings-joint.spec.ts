...
test.describe('NORMAL_TS001 - Joint Account (Live, Manually-Assisted)', () => {
  ...
  test('Normal Savings Account - Joint: full live journey through real final submission', async ({ page }) => {
    ...
    const OTP_SIGNAL_FILE = path.join(process.cwd(), '.nor-jnt-otp-input.txt');
    const CO_OTP_SIGNAL_FILE = path.join(process.cwd(), '.nor-jnt-co-otp-input.txt');
