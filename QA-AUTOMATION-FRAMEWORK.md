...
│
├── CLAUDE.md                     ← auto-loaded orchestrator routing for Claude Code
├── .claude/agents/ba-agent.md    ← BA Agent (Claude Code subagent) — Step 0
├── .github/agents/               ← agent definitions for VS Code / Copilot
│   ├── ba-agent.agent.md         ← BA Agent — Step 0
│   └── playwright-test-{planner,generator,healer}.agent.md   ← Steps 2, 4, 5
│
├── prompts/
│   ├── orchestrator.md           ← Main Orchestrator: task classification + agent routing
│   ├── ba_agent_prompt.md        ← Step 0 playbook: explore the live system → user story
│   └── master_prompt001.md       ← Steps 1–7 of the "agentic QA workflow" (see §8)
│
├── templates/                    ← output structures the BA Agent fills in
│   ├── user-story-template.md
│   └── exploration-log-template.md
│
...
`prompts/orchestrator.md` classifies an incoming request and routes it to the right agent; `prompts/master_prompt001.md` defines Steps **1–7**, and `prompts/ba_agent_prompt.md` defines Step **0**. This is the actual methodology, not just a historical artifact — follow it for the next module too.

| Step | Output | Where it lands |
|---|---|---|
| **0. Explore the existing system → user story** (BA Agent) | Verified current behaviour — user flow, field inventory, validation rules, business rules, positive/negative/edge scenarios, gaps and defect candidates — every claim evidence-tagged `[OBSERVED]`/`[REPO]`/`[INFERRED]`/`[NOT VERIFIED]`/`[GAP]`/`[DEFECT]`. Requirements are *never* invented; unverifiable areas are declared, not assumed | `user-stories/US_0NN_{Module}.md` + `specs/{SUITE_ID}-exploration-log.md` |
...
3. **Excel defect tracker** (`reports/{SUITE_ID}-defect-sheet.xlsx`) — for handoff to developers/PMs who live in Excel, not Markdown. Generated by `scripts/generate-{module}-defect-tracker.js` (ExcelJS): frozen header row, autofilter, conditional formatting on Status/Priority/Severity, and dropdown data-validation on Status/Priority/Severity/Type/Retested Result. Columns exactly match `prompts/master_prompt001.md`'s Step 7 spec: `Defect ID, Date, Instance, Module, Title, Description, Test Data / Required Info, Status, Screenshot / STR, Priority, Severity, Type, Retested Result, Developer Assigned, Resolved Date, Developer Comment, QA Comment, Changes Applied`.
